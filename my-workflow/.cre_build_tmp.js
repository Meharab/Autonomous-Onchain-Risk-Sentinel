...
var myAddress = "0x55F710a5509f4a8a8fE8a41dF476e51daD401454";
var Risk_PARAMS = parseAbiParameters("uint256 newRatio, uint256 riskScore, uint256 newSlope");
...
var fetchCEXResult = (nodeRuntime) => {
  ...
  const priceString = parsed.price;
...
var onCronTrigger = (runtime2) => {
  ...
  const debtContractCall = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: evmConfig.lendingProtocolAddress,
      data: debtCallData
    }),
    blockNumber: LAST_FINALIZED_BLOCK_NUMBER
  }).result();
  ...
  const collateralContractCall = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: evmConfig.lendingProtocolAddress,
